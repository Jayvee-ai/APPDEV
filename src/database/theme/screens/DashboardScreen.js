...
export default function DashboardScreen({ user, navigation }) {
  ...
  const loadDashboardData = useCallback(async () => {
    ...
    const counts = { pending: 0, under_review: 0, approved: 0, released: 0, rejected: 0 };
    ...
    <ScrollView style={styles.container} contentContainerStyle={styles.content}>
      ...
      <View style={styles.statsGrid}>
        {Object.entries(STATUS_META).map(([key, meta]) => (
          <Card key={key} style={[styles.statCard, isWide && styles.statCardWide]}>
      ...
      <View style={[styles.dashGrid, isWide && styles.dashGridWide]}>
        <Card style={[styles.recentCard, isWide && styles.recentCardWide]}>
            ...
            recentRequests.map(request => {
              ...
              return (
                <TouchableOpacity
                  ...
                  activeOpacity={0.85}
                >
                  <Text style={[styles.requestCode, styles.colRequest]}>{request.request_code}</Text>
                  <Text style={[styles.requestType, styles.colType]}>{cert?.name || 'Certificate Request'}</Text>
        ...
        <Card style={[styles.quickCard, isWide && styles.quickCardWide]}>
          <View style={styles.cardHeader}>
          ...
          <View style={styles.quickBody}>
            ...
            <TouchableOpacity style={styles.outlineButton} onPress={() => navigation.navigate('Track')}>
              <Text style={styles.outlineButtonText}>📋 Track My Requests</Text>
            </TouchableOpacity>

            <TouchableOpacity style={styles.ghostButton} onPress={() => navigation.navigate('Track')}>
              <Text style={styles.ghostActionText}>🔍 Verify a Certificate</Text>
            </TouchableOpacity>
          </View>
        </Card>
      </View>
      ...
      <Card>
        <View style={styles.cardHeader}>
          <Text style={styles.cardTitle}>Available Certificate Types</Text>
        </View>
        ...
        <View style={[styles.certGrid, isWide && styles.certGridWide]}>
          {availableCerts.map(cert => (
            <TouchableOpacity
              key={cert.id}
              style={[styles.certCard, isWide && styles.certCardWide]}
            ...
            >
              ...
              <View style={styles.certFooter}>
                <Text style={styles.certFee}>₱{cert.fee.toFixed(2)}</Text>
...
const styles = StyleSheet.create({
  ...
  pageTitle: { color: COLORS.text, fontSize: 22, fontWeight: '800', marginBottom: 6 },
  pageSubtitle: { color: COLORS.textMuted, fontSize: 13.5, marginBottom: 26 },
